...
const transformDatabaseByEnv = (settings) => {
  ...
  const transformedDatabases = _.reduce(databases, (finalDb, database, key) => {
    const {
      host, ports, username, password, replicaSet,
    } = database;
    ...
    dbUrl.push(ports.map(port => `${host}:${port}`).join(','));

    if (username && password) {
      dbUrl.push(`${username}:${password}@`);
    }

    // add replica set, so we can use latest "transaction" logic
    dbUrl.push(`/crowd-source-${environment}?replicaSet=${replicaSet}`);
