...
const runScripts = (scripts, url) => scripts.reduce((series, {
  dbName,
  collectionName,
  query,
  findAndUpdate,
  action,
}) => series.then(() => connect(url)
  .then((client) => {
    ...
    if (findAndUpdate) {
      ...
      const next = () => cursor.next()
        .then((doc) => {
          ...
          log(`Find and update on ${dbName} - ${collectionName} ${doc.id}`);
  ...
  .then(() => log('Done!'))
  .catch(err => log(err))), Promise.resolve());
