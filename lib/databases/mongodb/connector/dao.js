...
const findAndUpsertMany = function findAndUpsertMany(modelName, updateList) {
  logger('FindAndUpsertMany method called on [%s] with update list %o',
    modelName, updateList);

  const promiseArray = updateList.map((updateObj) => {
    const query = updateObj.id ? { _id: updateObj.id } : { _id: mongodbIdBuilder() };
...
const execute = function execute(modelNameOrMethod, methodName, ...args) {
  logger('Execute called for [%s] with method [%s] for args %o', modelNameOrMethod, methodName, ...args);
