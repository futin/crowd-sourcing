...
const findAndUpsertMany = function findAndUpsertMany(modelName, modelsList, identifiers = ['_id']) {
  ...
  const promiseArray = modelsList.map((model) => {
    ...
    return findOneAndUpdate(modelName, query, updateWithoutId, { upsert: true, runValidators: true });
  });
