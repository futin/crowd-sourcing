...
const findAllByDate = function findAllByDate(modelName, dateConfig, filter) {
  ...
  logger('FindAllByDate method called on [%s] with next dateFrom: %s and dateTo: %s', modelName, dateFrom, dateTo);
...
const findOneAndUpdate = function findOneAndUpdate(modelName, query, updateData, options) {
  logger(
    'FindOneAndUpdate method called on [%s] with next query: %o, updateData: %o and options %o',
    modelName,
    query,
    updateData,
    options,
...
const findAndUpsertMany = function findAndUpsertMany(modelName, modelsList, identifiers = ['_id'], additionalOptions) {
  logger(
    'FindAndUpsertMany method called on [%s] with models list %o and identifiers %o',
    modelName,
    modelsList,
    identifiers,
  );
...
const insertMany = function insertMany(modelName, listOfCollections, options) {
  logger(
    'InsertMany method called on [%s] for list of collections %o with options %o',
    modelName,
    listOfCollections,
    options,
  );
...
const updateOne = function updateOne(modelName, query, dataToUpdate, options) {
  logger(
    'UpdateOne method called on [%s] for query %o, update %o and options %o',
    modelName,
    query,
    dataToUpdate,
    options,
...
const updateMany = function updateMany(modelName, filter, dataToUpdate, options) {
  logger(
    'UpdateMany method called on [%s] for filter %o, update %o and options %o',
    modelName,
    filter,
    dataToUpdate,
    options,
