...
const {
  authConfig: {
    authRoute, jwtSecret, googleStrategyName, domainName, callbackUrl, expiresIn,
  },
  googleStrategy: { clientId: clientID, clientSecret, redirectUrl },
} = settings;
...
module.exports = (app) => {
  ...
  app.get(`/${callbackUrl}`, passport.authenticate(googleStrategyName), async (req, res, next) => {
    const { user } = req;
    const { emails } = user;
