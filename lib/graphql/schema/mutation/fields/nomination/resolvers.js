...
const addNominationResolver = (...params) =>
  combineResolvers(
    ...
    (_, { nominees }, { connector, user }) => {
      const transactionToExecute = connector.executeTransaction('insertNominationTransaction', {
        nominees,
...
const updateNominationFullResolver = (...params) =>
  combineResolvers(
    ...
    (parent, args, { connector, user, extras }) => {
      const transactionToExecute = connector.executeTransaction('updateNominationTransaction', {
        user,
        extras,
        ...args,
      });
