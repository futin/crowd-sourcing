 ...
 * @returns {Error}
 */
const validateNomineesPointsResolver = async (parent, { nominationId, nominees }, { connector, user }) => {
  ...
  return userInstance.pointsToAssign >= numberOfPointsState
    ? skip
    : new Error(`Invalid number of points. Allowed sum: ${user.pointsToAssign}, but received ${numberOfPointsState}`);
};
...
const validateNominationDateCreationAndIssuer = async (parent, { nominationId }, { connector, user }) => {
  ...
  if (startOfMonth > createdAt) { return new Error('Updates of old Nominations is not permitted'); }
