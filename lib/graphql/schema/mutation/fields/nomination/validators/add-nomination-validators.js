...
const {
  ...
  constants: {
    loggerNamespaces: { addNominationUtilsNamespace },
  },
} = require('@config');
...
const logger = debug(addNominationUtilsNamespace);
...
const validateSelfNominations = (parent, { nominees }, { user }) =>
  (nominees.map(nominee => nominee.userId).includes(user.id) ? new Error('Self-nomination is not allowed.') : skip);
