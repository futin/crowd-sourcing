...
const addCategory = {
  ...
  resolve: combineResolvers(roleValidatorResolver([roles.ADMIN]), (parent, args, { connector }) =>
    connector.insertOne('Category', args)),
};
...
const updateCategory = {
  ...
  resolve: combineResolvers(roleValidatorResolver([roles.ADMIN]), (parent, { name, description }, { connector }) =>
    connector.findOneAndUpdate('Category', { name }, { description })),
};
...
const deleteCategory = {
  ...
  resolve: combineResolvers(roleValidatorResolver([roles.ADMIN]), async (parent, { name }, { connector }) => {
